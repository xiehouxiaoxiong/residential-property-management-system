...
import { addPayment } from "@/apis/payment";
...
function sendPayWuYe() {
  getProprietorList("", "", 1, 1).then((res) => {
    let total = res.data.total;
    getProprietorList("", "", total, 1).then((res) => {
      let data = res.data.data.rows;
      data.forEach((item) => {
        let mon = item.rentArea * 1.4;
        let ss = {
          payStatus: "待支付",
          payType: "物业费",
          money: mon,
          payTime: moment(new Date()).format(timeFormat),
          userId: item.userId,
        };
        ...
        UpdateProprietor(item.userId, { wuyeArrearage: item.wuyeArrearage + mon,parkingArrearage:item.parkingArrearage,money:item.money }).then(
          (res) => {
            console.log(res);
            if (res.status == 200) {
            }
  ...
  <div style="width: 100%; justify-content: center; display: flex">
    <div class="headerStyleTable">
      <div style="display: flex">
        ...
        >
        <el-button type="primary" @click="sendPayWuYe">一键收物业费</el-button>
      </div>
