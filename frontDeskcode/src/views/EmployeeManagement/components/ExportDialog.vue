...
const updateFilterdata = async (val, fenRow, userName, employeeType, sex) => {
  await getEmployeeList(userName, employeeType, sex, fenRow, val).then((res) => {
...
const getInput = (input, selectd) => {
  ...
  employeeType = selectd.value[0];
  sex = selectd.value[1];
  updateFilterdata(1, fenRow, userName, employeeType, sex);
};
...
const clearInput = () => {
  ...
  updateFilterdata(changePage.pageCount, fenRow, userName, employeeType, sex);
};
