...
async function getSelectContent() {
  if (textTitle.indexOf("请假") !== -1 || textTitle.indexOf("出勤") !== -1) {
    emits("getInputText", inputFirst, inputSecond, selectText);
  } else {
...
function clearSelectContent() {
  ...
  selectText.value = [undefined, undefined];
...
<template>
  <div style="justify-content: space-between; width: 100%; display: flex">
    <div class="titleItem" v-if="textTitle.indexOf('导出') == -1">{{ textTitle }}</div>
    ...
    <div style="width: 98%; justify-content: center; display: flex">
      <div style="display: flex" class="headerStyleTable">
        <div style="display: flex">
          ...
          <el-input
            v-model="inputSecond"
            :placeholder="inputPlodSecond"
            style="width: 120px; margin-left: 10px; height: 30px"
            v-if="textTitle.indexOf('员工') === -1"
          />
