...
import {
  ...
  deleteAbsence,
  updateAbsence,
} from "@/apis/absenceInfo";
import { reactive, ref } from "vue";
import Tableheader from "./header.vue";
import Table from "./TableLa.vue";

import fenYe from "./fenyeAttendance.vue";
...
let cols1 = [
  ...
  {
    propName: "clockInDays",
    labelName: "打卡天数",
    width: "120px",
  },
  {
    propName: "absenceDays",
    labelName: "缺勤次数",
    width: "120px",
  },
];
...
function sendMessageToServer() {
  ...
  ws.onmessage = function (e) {
    ...
    updateFilterdataSocket(changePage1.currentPage, fenRow1, userName1, number);
  };
}
const updateFilterdataSocket = async (val, fenRow, userName, number) => {
  await getAbsenceList(userName1, number, fenRow1, val).then((res) => {
    console.log(userName1, number);
...
const updateFilterdata1 = async (val, fenRow, userName, number) => {
  await getAbsenceList(userName1, number, fenRow1, val).then((res) => {
    console.log(userName1, number);
    sendMessageToServer();
    tableData1.value = res.data.data.rows;
...
async function getForm1(params, title) {
  if (title.indexOf("新增") !== -1) {
    await AddAbsence(params).then((res) => {
      console.log(res);
    });
    await updateFilterdata1(changePage1.currentPage, fenRow1, userName1, number);
  } else if (title.indexOf("修改") !== -1) {
    await updateAbsence(editId1, params).then((res) => {});
    await updateFilterdata1(changePage1.currentPage, fenRow1, userName1, number);
  }
  addVisible1.value = false;
}
const getdeleteNoteId1 = async (e) => {
  await deleteAbsence(e);
  updateFilterdata1(changePage1.currentPage, fenRow1, userName1, number);
};
function getInput1(input, input2, selectd) {
  ...
  updateFilterdata1(1, fenRow1, userName1, number);
}
const clearInput1 = () => {
  inputText1.value = "";
  inputText2.value = "";
...
const getCurrentPage1 = (e) => {
  ...
  updateFilterdata1(e, fenRow1, userName1, number);
};
