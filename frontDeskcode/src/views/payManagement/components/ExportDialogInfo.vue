...
let props = defineProps({
  exportVisible: {},
  textTitle: {},
 select:{},inputPlod:{},editIcon:{},cols:{},inputPlodFirst:{}
});
let payType=''
let userName=''
...
function confirmExport() {
  ...
  console.log(data);
  data = data.map((item) => {
    let time = moment(item.payTime).format(timeFormat)
    return {
      userId:item.userId,
  ...
  if (lenSelectExcel > 0) {
    exportExcelSub(textTitle+"_excel", "unit", data, titleCol);
  } else {
    ElMessage({
      message: "请选择需要导出的数据",
      type: "warning",
...
const getInput = (input, selectd) => {
  ...
  updateFilterdata(1, fenRow,userName,payType)
...
<template>
  ...
  >
    ...
    <template #footer>
      <span class="dialog-footer" style="justify-content: center; display: flex;bottom:20px;position:absolute;text-align:center;width:100%">
