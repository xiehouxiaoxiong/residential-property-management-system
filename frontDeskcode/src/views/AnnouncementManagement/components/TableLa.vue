...
<template>
  <div style="font-weight: 700; font-size: 16px; width: 98%; height: 100%">
    ...
    >
      ...
      >
        <template #default="scope">
          <div v-if="item.propName === 'releaseTime'">
            {{ moment(scope.row[item.propName]).format(timeFormat) }}
          </div>
          <div
            v-if="
              item.propName !== 'announcementType' &&
              item.propName !== 'announcementStatus' &&
              item.propName !== 'picture' &&
              item.propName !== 'releaseTime'
            "
          >
          ...
          <div
            v-if="
              item.propName === 'announcementType' && scope.row.announcementType !== ''
            "
            class="typeAnnouncement"
            :style="{
              '---modalImage': selectType[scope.row.announcementType].image,
              '--modalColor': selectType[scope.row.announcementType].color,
            }"
          >
            {{ scope.row.announcementType }}
          </div>

          <div
            v-if="item.propName === 'announcementStatus'"
            class="statusAnnouncement"
            :style="{ '--modalColor': selectStatus[scope.row.announcementStatus] }"
          >
            {{ scope.row.announcementStatus }}
          </div>

          <div v-if="item.propName === 'picture'">
            <span style="color: #f3b805" @click="showImg(scope.row.picture)">查看</span>
          </div>
        </template>
      </el-table-column>

      <el-table-column fixed="right" label="操作" width="120" v-if="textTitle !== ''">
        <template #default="scope">
          <div style="color: #f3b805">
            <span @click="handleEdit(scope.$index, scope.row)" style="color: #f3b805">
              修改</span
            >
  ...
  <MonitorPicture
    :MonitorPictureVisible="MonitorPictureVisible"
    :url="urlList"
  ></MonitorPicture>
</template>
<style scoped>
