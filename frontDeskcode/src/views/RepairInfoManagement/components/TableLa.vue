      ...
      >
        ...
        >
          <template #default="scope">
            <div
              v-if="
                item.propName !== 'applicationtime' && item.propName !== 'auditStatus'
              "
            >
              {{ scope.row[item.propName] }}
            </div>
            <div v-if="item.propName === 'applicationtime'">
              {{ moment(scope.row.applicationtime).format(timeFormat) }}
            </div>
            <div v-if="item.propName === 'auditStatus'">
              <div
                class="statusAudit"
                :style="{ '--modalColor': statusColor[scope.row[item.propName]] }"
              >
                {{ scope.row[item.propName] }}
              </div>
            </div>
          ...
          <template #default="scope">
            <div style="color: #f3b805">
              <span
                ...
                style="color: #f3b805"
                v-if="
                  scope.row.repairProgress !== '完工' &&
                  scope.row.repairProgress !== '评价'
                "
              >
                审核</span
              >
