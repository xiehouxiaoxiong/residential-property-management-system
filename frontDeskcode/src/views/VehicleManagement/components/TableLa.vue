      ...
      >
        ...
        >
          <template #default="scope">
            <div
              v-if="
                item.propName === 'parkingStartTime' || item.propName === 'parkingEndTime'
              "
            >
              {{ moment(scope.row[item.propName]).format(timeFormat) }}
            </div>
            <div v-if="item.propName === 'auditResult'">
              <div v-if="scope.row.auditResult === '待审核'" style="color: yellow">
                {{ scope.row.auditResult }}
              </div>
              <div v-if="scope.row.auditResult === '通过'" style="color: green">
                {{ scope.row.auditResult }}
              </div>
              <div v-if="scope.row.auditResult === '不通过'" style="color: red">
                {{ scope.row.auditResult }}
              </div>
              <div v-if="scope.row.auditResult === '申请成功'" style="color: black">
                {{ scope.row.auditResult }}
              </div>
              <div v-if="scope.row.auditResult === '已失效'" style="color: gray">
                {{ scope.row.auditResult }}
              </div>
            </div>
            <div
              v-if="
                item.propName !== 'parkingStartTime' &&
                item.propName !== 'parkingEndTime' &&
                item.propName !== 'auditResult'
              "
            >
          ...
          <template #default="scope">
            <div style="color: #f3b805">
              ...
              <span
                @click="handleEdit(scope.$index, scope.row)"
                style="color: #f3b805"
                v-if="textTitle === '车位申请' && scope.row.auditResult !== '通过'"
              >
                审核</span
              >
              <span
